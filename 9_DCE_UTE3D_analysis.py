...
%run "/Users/alex/AlexBadea_MyExperiments/DennisTurner/Code/9_DCE_UTE3D_analysis.py" \
...
%run "/Users/alex/AlexBadea_MyExperiments/DennisTurner/Code/9_DCE_UTE3D_analysis.py" \
  ...
  --roi_mm "T2:3.58,-0.03,1.33,0.4" \
  --roi_mm "AIF1:-0.37,-5.10,-1.80,0.2" \
  --roi_mm "AIF3:3.60,-5.70,0.70,0.2" \
  --auto_aif 0 \
  --outdir "/Users/alex/AlexBadea_MyExperiments/DennisTurner/Data/100925/ute3d_mm_outputs_AB"
...
- roi_curves.csv, roi_pe_curves.png, roi_raw_curves.png
- roi_summary.csv  (PE_max, TTP, slopes per ROI)
- aif_sanity.csv (if any ROIs start with "AIF")
...
AIF handling
- --use_aif <ROI> forces a specific ROI to be saved as the AIF curve CSV.
- Otherwise, the script auto-picks the “optimal” AIF: earliest TTP, tie→highest PE_max.
- If multiple share the earliest TTP, it additionally writes AIF_mean_early_curve.csv.
...
def force_ras(img):
    """Return a copy reoriented to RAS+. Keeps data 4D. Works with newer or older nibabel."""
    # Fast path (newer nibabel exposes nib.as_closest_canonical)
...
def aif_sanity_report(R, baseline_frames, time_spacing, out_csv, aif_prefix="AIF", early_frac=0.25,
    ...
    # precompute TTPs for earliest check
    ttp_per = {}
    for rn in aif_names:
        pe = np.asarray(R[rn]["pe"], float)
        ttp_per[rn] = int(np.nanargmax(pe)) if pe.size else 10**9
    if len(ttp_per) == 0:
        return rows
    earliest_ttp = min(ttp_per.values())
    ...
    for rn in aif_names:
        ...
        is_earliest = (ttp_idx == earliest_ttp)
        ...
        if verbose:
            print(f"[AIF sanity] {rn}: TTP={ttp_min:.2f} min "
                  ...
                  f"{'washout✓' if washout_ok else 'washout✗'} "
                  f"{'earliest✓' if is_earliest else ''} → {verdict}")
...
def _save_aif_curve(outdir, name, raw, pe, dt, file_suffix=""):
    """Write a single AIF curve CSV and tag run_meta."""
    suf = f"_{file_suffix}" if file_suffix else ""
    path = Path(outdir) / f"{name}{suf}_curve.csv"
    with path.open("w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["frame","time_sec","time_min","raw","PE"])
        T = len(raw)
        for i in range(T):
            t = i*float(dt)
            w.writerow([i, t, t/60.0, float(raw[i]), float(pe[i])])
    # also mark in meta (if exists)
    meta_p = Path(outdir) / "run_meta.json"
    if meta_p.exists():
        meta = json.loads(meta_p.read_text())
        meta[f"chosen_aif_saved{suf}"] = name
        meta_p.write_text(json.dumps(meta, indent=2))
    print(f"[AIF] Saved {name}{suf} to {path}")

def pick_and_save_aif(R, outdir, dt, aif_prefix="AIF", make_early_mean=True, file_suffix=""):
    """
    From ROIs in R whose name starts with aif_prefix:
      1) Find earliest TTP; among ties, choose highest PE_max.
      2) Save its curve CSV.
      3) If multiple share the earliest TTP and make_early_mean=True,
         also save AIF_mean_early.
    Returns the chosen AIF name and (optional) mean name.
    """
    if not R:
        return None, None
    aifs = [rn for rn in R if rn.upper().startswith(aif_prefix.upper())]
    if not aifs:
        return None, None

    def ttp_idx_of(rn):
        pe = np.asarray(R[rn]["pe"], float)
        return int(np.nanargmax(pe)) if pe.size else 10**9

    ttp_by = {rn: ttp_idx_of(rn) for rn in aifs}
    min_ttp = min(ttp_by.values())
    earliest = [rn for rn in aifs if ttp_by[rn] == min_ttp]

    def pe_max_of(rn):
        pe = np.asarray(R[rn]["pe"], float)
        return float(pe[np.nanargmax(pe)]) if pe.size else -np.inf

    best = max(earliest, key=pe_max_of)
    _save_aif_curve(outdir, best, R[best]["raw"], R[best]["pe"], dt, file_suffix)

    mean_name = None
    if make_early_mean and len(earliest) >= 2:
        raw_stack = np.vstack([R[r]["raw"] for r in earliest])
        pe_stack  = np.vstack([R[r]["pe"]  for r in earliest])
        raw_m = raw_stack.mean(axis=0)
        pe_m  = pe_stack.mean(axis=0)
        mean_name = "AIF_mean_early"
        _save_aif_curve(outdir, mean_name, raw_m, pe_m, dt, file_suffix)
    return best, mean_name
...
def main():
    ...
    # AIF override
    ap.add_argument('--use_aif', default=None,
                    help='Exact ROI name to use as AIF (e.g., AIF2 or AIF_mean_early). If set, overrides auto pick.')
    ...
    if data.ndim != 4:
        raise RuntimeError(f'Expected 4D NIfTI; got {data.shape}')
    if args.flip_time:
        data = data[..., ::-1]
    ...
    with (outdir/'global_curve.csv').open('w', newline='') as f:
        ...
        for i,(r,p) in enumerate(zip(global_raw, global_pe)):
            t = i*args.time_spacing
            w.writerow([i, t, t/60.0, float(r), float(p)])

    times = np.arange(global_pe.size)*args.time_spacing
    plt.figure(figsize=(7.4,4.3))
    plt.plot(times/60.0, global_pe)
    if args.injection_time_sec is not None:
        plt.axvline(args.injection_time_sec/60.0, linestyle='--')
    plt.xlabel('Time (min)')
    plt.ylabel('% enhancement (masked mean)')
    plt.title('Global Mean DCE Curve')
    plt.tight_layout()
    plt.savefig(outdir/'global_curve.png', dpi=200)
    plt.close()
    ...
    # ---------- ROI extraction ----------
    R = {}
    chosen_aif, chosen_aif_mean = None, None
    if rois_mm_final:
        ...
        if args.save_roi_masks:
            ...
            with (outdir/'roi_set_mm.json').open('w') as f:
        ...
        # ----- AIF selection / saving -----
        if args.use_aif:
            if args.use_aif in R:
                _save_aif_curve(outdir, args.use_aif, R[args.use_aif]['raw'], R[args.use_aif]['pe'], args.time_spacing)
                chosen_aif = args.use_aif
                print(f"[AIF] Using user-specified AIF: {chosen_aif}")
            else:
                print(f"[AIF] Requested '{args.use_aif}' not found among ROIs. Auto-picking instead.")
                chosen_aif, chosen_aif_mean = pick_and_save_aif(R, outdir, args.time_spacing,
                                                                aif_prefix="AIF", make_early_mean=True)
        else:
            chosen_aif, chosen_aif_mean = pick_and_save_aif(R, outdir, args.time_spacing,
                                                            aif_prefix="AIF", make_early_mean=True)
    ...
    meta_out = {
        ...
        'chosen_aif': chosen_aif,
        'chosen_aif_mean_early': chosen_aif_mean
    }
